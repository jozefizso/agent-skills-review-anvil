...
On a `success` outcome the helper refreshes the full prior-feedback ledger before editing the comment. If lookup fails or the refreshed history changes the compact report's finding inventory, it changes the update to `outcome=failure` and explains why; it never publishes an unfiltered or internally stale success report, and it still avoids leaving a dangling "starting" comment. After the successful edit, it resolves entries from `<REPORT_PATH>.resolutions.json` only when the artifact's `head_sha`—local `HEAD` after the engine's final accepted fix commit—equals the live pushed PR head and the live open root carries the matching terminal review-anvil marker. A fixed/stale finding may resolve; an accepted author explanation also requires a live PR-author reply. It never resolves human-authored/non-anvil threads.
...
Before calling `post-update` on a success outcome, read `<REPORT_PATH>.followups.json` (if present) and surface its entries to the user — the script deletes it afterwards. The script cleans up the five transient report artifacts (`<REPORT_PATH>`, `.inline.json`, `.resolutions.json`, `.approval.json`, `.followups.json`) and removes their self-ignored `.review-anvil/` directory when no other report artifacts remain. If delivery succeeds but thread resolution fails, it retains `.resolutions.json`; retry it with `bash <helper-path> resolve "$HOST" "$OWNER" "$REPO" "$N" "<resolutions-path>"`. Retained proof bundles live in a separate host-created private directory outside the worktree and are unchanged.

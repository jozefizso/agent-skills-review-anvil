...
- Earlier review comments: none | <open> open, <resolved> closed, <reported> summary-only, <author-resolved> author-resolved (skipped), <author-explanation-accepted> author explanations accepted, <suppressed> skipped; <still-present>/<fixed>/<stale> after checking
...
2. Write a sibling `<report_path>.inline.json`: an array of GitHub PR review comment payloads for findings with both `file` and `line` —
   ...
   Single line → `{"line": N, "side": "RIGHT"}`; range `<N>-<M>` → `{"start_line": N, "line": M, "side": "RIGHT", "start_side": "RIGHT"}`. Findings not emitted inline stay in the markdown report only; no eligible anchored findings → `[]`. Each visible body uses an unlabeled diagnosis followed by `Suggestions:` with one bullet per independently implementable change. Prefer separate bullets whenever actions remain clear on their own. Default to starting required work with its action verb. A deliberate collaborative request may use a courtesy wrapper sparingly when coordination or tone benefits. Never use one as a stock opener throughout the review. End a direct imperative as a statement rather than a question. Keep optional low/nit work as `Consider …`; ask a real question only for a source-level unresolved choice. End the body with exactly one `<!-- review-anvil: id=<complete-id> severity=<severity> area=<area> -->` metadata marker; no prose follows it.
...
3. Write a sibling `<report_path>.resolutions.json` with review-anvil threads that the final synthesis proved obsolete:

   ```json
   {
     "head_sha": "<exact PR head reviewed by this final synthesis>",
     "threads": [
       {
         "thread_id": "PRRT_kwDOExample",
         "finding_id": "RAV-RUN2-R1-F003",
         "disposition": "fixed | stale | author-explanation-accepted",
   ...
   Use the captured PR `HEAD_SHA` for read-only PR runs. For `review-anvil-improve-pr`, use `git rev-parse HEAD` after the final accepted fix commit and before push; that is the exact snapshot whose code and prior feedback produced these dispositions. Write an empty `threads` array when no thread qualifies. Include an item only when PR REVIEW HISTORY supplies `thread=<id>; anvil=true`, the thread is still open, and current-code evidence proves one of these outcomes: the defect is fixed; the concern is stale and no longer applies; or a PR-author reply gives a technically valid explanation. `outdated` alone is not enough. A reply's assertion is not proof; validate it against the repository. Never include a human-authored/non-anvil thread, an unresolved disagreement, a merely moved anchor, or a concern that still exists. The posting helper refreshes GitHub state, refuses every non-empty artifact whose `head_sha` differs from the live PR head, matches `finding_id` to the live root's terminal review-anvil marker, requires a live PR-author reply for `author-explanation-accepted`, and resolves only those validated thread IDs after successful delivery.

4. Write a sibling `<report_path>.approval.json` so the PR-posting helper can choose the GitHub review event (review-only PR runs; for other runs write `{"event": "COMMENT"}` or omit the file — the helper defaults to COMMENT):
...
`author-resolved` and `author-explanation-accepted` history do not affect the review decision or approval.
